...
def fetch_and_clean_html(url: str, client) -> str:
    try:
        ...
        try:
            response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'nvidia/nemotron-3-nano-30b-a3b:free'), messages=[{'role': 'system', 'content': SYSTEM_PROMPT}, {'role': 'user', 'content': f'Render this HTML content: {clean_input}'}], stream=False)
