...
def fetch_and_clean_html(url: str, client) -> str:
    try:
        ...
        try:
            response = client.chat.send(model='google/gemini-2.5-pro', messages=[{'role': 'system', 'content': SYSTEM_PROMPT}, {'role': 'user', 'content': f'Render this HTML content: {clean_input}'}], stream=False)
