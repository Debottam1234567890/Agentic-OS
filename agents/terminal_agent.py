...
def map_architecture(current_dir: str, target_path: str = "."):
    ...
    # The Walker: Initiate a for loop using os.walk
    for root, dirs, files in os.walk(target_path):
        
        # In-Place Directory Pruning: iterate backwards to safely remove items
        for d in reversed(dirs):
            if d in guardrail_list:
                dirs.remove(d)
                
        # The File Filter: iterate through files
        for file_name in files:
            if not file_name.endswith(".py"):
                continue
                
            # Path Construction: combine root and file name
            file_path = os.path.join(root, file_name)
            
            # Phase 3: The AST Parsing Logic
            # Safe File Opening
            try:
...
def patch_file(current_dir, file_path: str, start_line: int, end_line: int, replacement_code: str):
    ...
    # Phase 2: Memory Loading & Slicing
    try:
        with open(file_path, "r", encoding="utf-8") as f:
    ...
    # Phase 3: The Compiler Guardrail
...
def execute_terminal_agent(user_input: str, current_dir: str, visible_files: str, recent_history: str, client, update_callback) -> dict:
    ...
    # Phase 1: Create local sandbox folder if not exists
    sandbox_dir = get_sandbox_dir(current_dir)
        ...
        if tool_name == "write_file":
            ...
            try:
                tool_result = patch_file(
                    ...
                    tool_args.get("replacement_code", "")
                )
            except Exception as e:
