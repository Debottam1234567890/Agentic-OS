...
def stream_web_agent(user_input: str, client, set_output, append_output) -> dict:
    """Extracts query, scrapes web, and streams synthesis."""
    
    query_response = client.chat.send(
    ...
    if "No search results found" in live_data:
        ...
        set_output(err_msg)
        return {
            "agent_title": "Web Agent",
            "agent_color": "#FFFF00", # Yellow glow
            "command": f"Web Search: {search_query}",
            "output": err_msg
        }
    ...
    response = client.chat.send(
        model="qwen/qwen3-32b",
        messages=[
            {"role": "system", "content": WEB_SYNTHESIZER_PROMPT},
            {"role": "user", "content": f"User Intent: {user_input}\n\nLive Web Data: {live_data}\n\nAnswer the user's intent using ONLY the live web data provided."}
        ],
        stream=True
    )
