...
def stream_web_agent(user_input: str, client, set_output, append_output) -> dict:
    query_response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'nvidia/nemotron-3-nano-30b-a3b:free'), messages=[{'role': 'system', 'content': WEB_EXTRACTOR_PROMPT}, {'role': 'user', 'content': user_input}])
    ...
    set_output(base_msg)
    response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'nvidia/nemotron-3-nano-30b-a3b:free'), messages=[{'role': 'system', 'content': WEB_SYNTHESIZER_PROMPT}, {'role': 'user', 'content': f"User Intent: {user_input}\n\nLive Web Data: {live_data}\n\nAnswer the user's intent using ONLY the live web data provided."}], stream=True)
