...
def list_checkpoints(filename: str) -> list:
    """Return all checkpoint backup names for a given filename, newest first."""
...
def rollback_file(filename: str) -> str:
    """Roll back a file to its most recent checkpoint and show the diff."""
    if not os.path.exists(VAULT_DIR):
    ...
    # Resolve target: check sandbox first, then cwd
    ...
    try:
        ...
        # Find the most recent backup that is DIFFERENT from the current file
        ...
        # Diff: what changed from backup → current (what we're undoing)
        diff = difflib.unified_diff(
            current_lines,
            backup_lines,
            fromfile=f"Current ({filename})",
            tofile=f"Restored ({most_recent_backup_name})",
        )
        ...
        return (
            f"[green]✔ Successfully rolled back to {most_recent_backup_name}[/green]\n\n"
            f"{final_diff_output}"
        )
