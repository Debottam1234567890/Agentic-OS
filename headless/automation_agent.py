...
AUTOMATION_PROMPT = """\
You are a professional Web Automation Agent controlling a headless browser.
...
def execute_web_automation(user_input: str, client, update_callback) -> dict:
    ...
    try:
        while step_counter < 50:
            ...
            # Robust JSON extraction: strip fences, then find first { to last }
            cleaned = raw_content
            ...
            if action is None:
                # Ask the LLM to retry with valid JSON instead of aborting
                messages.append({"role": "assistant", "content": raw_content})
                messages.append(
                    {
                        "role": "user",
                        "content": "ERROR: Your last response was not valid JSON. Output ONLY a single JSON object: {\"thought\": \"...\", \"tool\": \"...\", \"args\": {...}}",
                    }
                )
                step_counter += 1
                continue
            ...
            if tool == "navigate":
                ...
                result = browser.type_text(
                    str(args.get("node_id", "")), args.get("text", "")
                )
                ...
                final_output["output"] = args.get(
                    "result", "Completed without result."
                )
                break
            else:
