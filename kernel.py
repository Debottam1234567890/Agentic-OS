...
class KernelOS(App):
    ...
    def process_journal_async(self, message: str):
        ...
        try:
            response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'nvidia/nemotron-3-nano-30b-a3b:free'), messages=[{'role': 'system', 'content': prompt}, {'role': 'user', 'content': message}], stream=False)
    ...
    def generate_quiz(self, topic: str):
        ...
        try:
            response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'nvidia/nemotron-3-nano-30b-a3b:free'), messages=[{'role': 'user', 'content': prompt}], stream=False)
