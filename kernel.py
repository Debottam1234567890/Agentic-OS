...
class KernelOS(App):
    ...
    def process_journal_async(self, message: str):
        ...
        try:
            response = client.chat.send(model='qwen/qwen3-32b', messages=[{'role': 'system', 'content': prompt}, {'role': 'user', 'content': message}], stream=False)
    ...
    def generate_quiz(self, topic: str):
        ...
        try:
            response = client.chat.send(model='google/gemini-2.5-pro', messages=[{'role': 'user', 'content': prompt}], stream=False)
