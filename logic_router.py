...
def route_intent(user_input: str, client) -> str:
    ...
    _diag('=' * 80)
    try:
        response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'qwen/qwen3-coder:free'), messages=[{'role': 'system', 'content': ROUTER_PROMPT}, {'role': 'user', 'content': user_input}], stream=False)
