...
def analyze_image(file_path: str, client, user_query: str, code_context: str='') -> str:
    with open(file_path, 'rb') as f:
        ...
        response = client.chat.send(model=os.environ.get('AGENTIC_OS_MODEL', 'nvidia/nemotron-3-nano-30b-a3b:free'), messages=[{'role': 'user', 'content': content_array}], stream=False)
